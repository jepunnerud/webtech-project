  ...
  return (
    <div className={styles.container}>
      ...
      <p className={styles.filterBanner}>
        Current filter:&nbsp;
        {filterType ? `${filterType[0].toUpperCase() + filterType.slice(1)} – ${filterValue}` : "None"}
      </p>
      ...
      <div className={styles.descriptionControls}>
        <div className={styles.buttonGroup}>
          ...
          {(["easy", "medium", "advanced"] as const).map((l) => (
            <button key={l} onClick={() => setTextLevel(l)} className={textLevel === l ? styles.active : ""}>
        ...
        <div className={styles.buttonGroup}>
          ...
          {(["short", "extended"] as const).map((l) => (
            <button key={l} onClick={() => setTextLength(l)} className={textLength === l ? styles.active : ""}>
              {l[0].toUpperCase() + l.slice(1)}
            </button>
          ))}
        </div>
        ...
        <table className={styles.statsTable}>
          ...
          <tbody>
            {player.teams.map((t, i) => (
              <tr key={i}>
                <td>
                  <Link href={`/museum/all/${player.id}?filter=team&value=${slug(t.club)}`} className={styles.linkChip}>
                    {t.club}
                  </Link>
                </td>
                <td>{t.appearances}</td>

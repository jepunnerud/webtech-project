...
const qrPromises = players.map((player) => {
  return new Promise((resolve, reject) => {
    ...
    if (
      ...
      typeof player.long_description_wiki !== 'string' ||
      player.long_description_wiki.trim() === ''
    ) {
      console.warn(
        `Skipping QR for ${player.name} due to missing or invalid long_description_wiki.`
      )
    ...
    console.log(
      `Generating QR for ${player.name} (Content: ${qrContent.substring(0, 50)}...) into ${qrPath}...`
    )
